...
void test3()
{
    ...
    // write the sequence
    seq->write(stdout, false); // in beats
	/* score should include
		...
		69 @ m 0.75 = 1.8s
	 */
    seq->write(stdout, true);  // in seconds
}
...
void test4()
{
	...
	fclose(file);
	delete seq;
	file = fopen("test4.mid", "rb");
	if (!file) {
		printf("could not open test4.mid for reading");
		return;
	}
	seq = new Alg_seq(file, true); // read midi file
	fclose(file);
	seq->write(stdout, false);
}
...
{
    Alg_seq_ptr seq = make_score();
	FILE *file = fopen("test5.gro", "w");
	if (!file) {
		printf("could not open test5.gro for writing");
		return;
	}
	seq->write(stdout, true);
	seq->write(file, true);
	fclose(file);
	delete seq;
	file = fopen("test5.gro", "r");
	if (!file) {
		printf("could not open test5.gro for reading");
		return;
	}
	seq = new Alg_seq(file, false); // read midi file
	fclose(file);
...
void test21() // TEST 21 -- test paste
{
    ...
    copy_buffer->write(stdout, false);
